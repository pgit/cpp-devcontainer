...
RUN BV=$(echo "$BOOST_VERSION"|tr . _) && \
    ...
    ./b2 toolset=clang cxxflags="-stdlib=libc++" linkflags="-stdlib=libc++" -j$(nproc) \
...
# fmt
#
# /fmt-10.2.1/test/format-impl-test.cc:310:20: error:
#   'is_floating_point' cannot be specialized: Users are not allowed to specialize this
#   standard library entity [-Winvalid-specialization]
#
...
RUN wget https://github.com/fmtlib/fmt/releases/download/${FMT_VERSION}/fmt-${FMT_VERSION}.zip && \
    ...
    mkdir build && \
    cd build && \
    CXXFLAGS="$CXXFLAGS -Wno-invalid-specialization" cmake .. && \
